...
xVec=linspace(-pi/2,pi/2,1000);

kk=(length(K)-1)/2+1;
yData=zeros(1,length(xVec));
for kk=1:length(K)
    % Get basis vectors
    v=vecStatic0(:,:,kk);
    ...
    un=makeBlochState(vn,kk);
    
    yPData=un(xVec);      
    if yData(floor(length(xVec)/2)+10)<0
        yData=-yData;
    end        
    
    yData=yData.*exp(0*1i*K(kk)*xVec/pi);
    
    N=sum(yPData.*conj(yPData));
    yPData=yPData/sqrt(N);
    yData=yData+yPData;
end

yData=yData/sqrt(length(K));

sum(yData.*conj(yData))
p=plot(xVec/pi,real(yData.*conj(yData)),'LineWidth',2); 
